...
const ItemCard = ({ item, showActions = false, onDelete }) => {
  ...
  const apiOrigin = configuredApiUrl?.includes('localhost:5000')
  ...
  return (
    <div className="item-card">
      <div className="item-image-container">
        {imageSrc ? (
          <img src={imageSrc} alt={item.item_name || item.name || 'Item'} className="item-image" />
        ) : (
      ...
      <div className="item-details">
        ...
        <h3>{item.item_name || item.name || 'Unnamed Item'}</h3>
